...
# Scope-minimal: send-class scopes ONLY, no reads (the delta invariant).
...
async def list_drafts(
    ...
    For each draft returns its Draft ID (the handle to pass to
    send_gmail_message(draft_id=...)), Thread ID, recipient, subject, and a snippet —
    enough to disambiguate drafts that share a subject, to recover a draft whose ID
    rotated after a Gmail-UI edit, and to detect duplicates in a thread.
    ...
    lines.append("\nTo send one: send_gmail_message(draft_id='<Draft ID>').")
    return "\n".join(lines)
