...
Signed download URLs (opt-in, for hosted deployments): with `WORKSPACE_MCP_SIGNED_ATTACHMENT_URLS=true`, `get_gmail_attachment_content`, `get_drive_file_download_url` and `get_gmail_message_content(full=true)` return a short-lived signed URL (`/attachments/signed/{token}`) instead of writing the file to disk or inlining base64. The route verifies the HS256 token, recovers the owner's Google credentials from the server's session store and streams the bytes from Google; nothing is stored, and the link is clamped to the owner's access-token life (at most 15 minutes). The signing key is derived from `FASTMCP_SERVER_AUTH_GOOGLE_JWT_SIGNING_KEY` (else `GOOGLE_OAUTH_CLIENT_SECRET`), and the link uses `WORKSPACE_EXTERNAL_URL` as its base, where the route must be publicly reachable (carve it out of any authenticating proxy). Single-process: the instance that serves the link must be the one holding the owner's session. When a link cannot be issued the tool says so and falls back to the standard path. Off by default; when off, nothing changes.
