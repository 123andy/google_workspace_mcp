...
Signed download URLs (opt-in, for hosted deployments): with `WORKSPACE_MCP_SIGNED_DOWNLOAD_URLS=true`, `get_gmail_attachment_content`, `get_drive_file_download_url` and `get_gmail_message_content(full=true)` return a short-lived signed URL (`/attachments/signed/{token}`) instead of writing the file to disk or inlining base64. The route verifies the HS256 token, recovers the owner's Google credentials (its in-process session store first, then the persistent credential store) and fetches the bytes from Google (Drive streamed in bounded chunks, Gmail bodies fetched whole); the route never writes to either store, and a credential it must refresh is refreshed in memory only. A link lives at most 15 minutes; without a refresh token it is clamped to end before the access token stops being usable. The signing key is derived from `FASTMCP_SERVER_AUTH_GOOGLE_JWT_SIGNING_KEY` (else `GOOGLE_OAUTH_CLIENT_SECRET`) under a salt of its own. The link uses `WORKSPACE_EXTERNAL_URL` as its base, where the route must be publicly reachable (carve it out of any authenticating proxy); the token travels in the URL path, so it appears in access logs unless `/attachments/signed/*` is redacted or access logging is off. Multi-replica deployments need a shared credential store, since any replica may receive the GET. When a link cannot be issued the tool says so and falls back to the standard path. Off by default; when off, nothing changes (the flag is ignored, with a startup log line, on stdio).
