...
Office files (`.docx`, `.xlsx`, `.pptx`) are ZIP archives, so the ceiling above bounds only their compressed size. Text extraction separately limits how many bytes of XML one file may expand to: `WORKSPACE_MCP_MAX_OFFICE_XML_BYTES` (default `26214400`, 25 MiB). A file beyond the limit is reported as too large to extract instead of being read. This limits expanded bytes, not memory: the parsed XML tree measured roughly 14 to 30 times the XML size, so lower the value on small containers. `0` removes the limit; invalid or negative values fail server startup.
