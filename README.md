...
- **PostgreSQL OAuth proxy storage** (this fork) - set `WORKSPACE_MCP_OAUTH_PROXY_STORAGE_BACKEND=postgres` and `WORKSPACE_MCP_OAUTH_PROXY_POSTGRES_DSN` (install `workspace-mcp[postgres]`). An explicit backend always wins; with none set, a set `..._VALKEY_HOST` implies Valkey, then a set `..._POSTGRES_DSN` implies Postgres. Records are Fernet-encrypted as for disk/Valkey. Optional: `..._POSTGRES_TABLE` (`workspace_mcp_kv`), `..._POSTGRES_SWEEP_INTERVAL_SECONDS` (900; Postgres does not evict expired rows itself, so the server sweeps them opportunistically), `..._POSTGRES_POOL_MIN`/`_MAX` (1/5).
...
Signed download URLs (opt-in, for hosted deployments): with `WORKSPACE_MCP_SIGNED_DOWNLOAD_URLS=true`, `get_gmail_attachment_content`, `get_drive_file_download_url` and `get_gmail_message_content(full=true)` return a short-lived encrypted link (`/attachments/signed/{token}`) that streams the bytes from Google on demand, instead of writing the file to disk or inlining base64. The switch is off by default and needs `WORKSPACE_EXTERNAL_URL` as the links' public base plus key material (`GOOGLE_OAUTH_CLIENT_SECRET` or `FASTMCP_SERVER_AUTH_GOOGLE_JWT_SIGNING_KEY`) to derive the link key; with the switch on over streamable-http, the server refuses to start when either is missing. See the **[Advanced Deployment guide](https://workspacemcp.com/docs/deployment?utm_source=github.com&utm_medium=referral&utm_campaign=readme&utm_content=deploy-advanced)** for proxy, replica and logging setup.

**Short download links** (this fork): while signed download URLs are on, the tools hand out the claim-check form `{external_base}/dl/{handle}` (~22-char random handle) instead of the ~600-character `/attachments/signed/{token}` link. The full encrypted token is held in the shared KV store (`WORKSPACE_MCP_OAUTH_PROXY_*` backend, encrypted at rest; in-process store otherwise) for the link's remaining life, and `/dl/{handle}` serves it through exactly the same route logic, checks and headers as the long form — it never redirects to the long URL. An unknown or expired handle answers as a bad token does. If the handle cannot be stored the long link is returned instead, so a link is never dead. On by default; `WORKSPACE_MCP_SHORT_SIGNED_URLS=false` keeps the long form (e.g. multi-replica deployments without a shared backend). Carve `/dl/*` out of any authenticating proxy alongside `/attachments/signed/*`.
